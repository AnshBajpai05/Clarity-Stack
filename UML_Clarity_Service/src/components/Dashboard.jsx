...
SHAPES TO USE — use ONLY these exact "type" values:
  ...
  "standard.Link"       — arrow from Actor→UseCase or UseCase→UseCase. 
                          IMPORTANT: Use labels like "<<include>>" or "<<extend>>" for UseCase→UseCase links.

CRITICAL: Focus ONLY on core functional goals. Ignore technical setup, metadata, or document structure. If multiple stories describe the same high-level feature, MERGE THEM.
...
const buildPrompt = (req, srsContext = '') => {
    ...
    const RICHNESS = {
        ...
        generic: 'Generate a DETAILED diagram with at least 4-5 entities and meaningful relationships between them. DEDUPLICATION: Merge semantically identical entities. If "Login" and "User signs in" appear, create only ONE node. Do NOT repeat goals. Think about what a complete system architecture would look like.',
    };
...
const Dashboard = () => {
    ...
    const [activeDiagramType, setActiveDiagramType] = useState(null); // e.g. 'use_case', 'activity', 'dfd'
    ...
    const [stage3Files, setStage3Files]       = useState([]); // Dynamic files from backend
    ...
    // Maps display-name → backend doc_id (stem of the intelligence JSON filename)
    const DOC_ID_MAP = {
    ...
    React.useEffect(() => {
        ...
        const handleMessage = async (event) => {
            if (event.data && event.data.type === 'SRS_CONTEXT') {
                const { doc_id, markdown } = event.data.payload;
                console.log("[Dashboard] Received SRS_CONTEXT from parent for", doc_id);
                setLoading(true);
                setParsingStatus('Importing SRS from ClarityStack...');
                try {
                    const blob = new Blob([markdown], { type: 'text/markdown' });
                    const file = new File([blob], `${doc_id}.md`, { type: 'text/markdown' });
                    const formData = new FormData();
                    formData.append('file', file);

                    const resp = await fetch(`${API_BASE}/api/upload-readme`, {
                        method: 'POST',
                        body: formData
                    });
                    ...
                    setLoading(false);
                    setParsingStatus(null);
                    setError('Error importing SRS: ' + err.message);
                }
            }
        };
    ...
    const fetchLiveDocs = async () => {
        try {
            ...
            console.warn('SRS backend offline (list documents)');
        }
    };

    const fetchStage3Files = async () => {
        try {
            const resp = await fetch(`${API_BASE}/api/list-stage3`);
            if (resp.ok) {
                const data = await resp.json();
                setStage3Files(data || []);
            }
        } catch (e) {
            console.warn('SRS backend offline (list stage3)');
        }
    };
    ...
    const pollParsingStatus = (docId) => {
        ...
        const interval = setInterval(async () => {
            try {
                const resp = await fetch(`${API_BASE}/api/document/${docId}/status`);
                ...
                if (data.status === 'processing') {
                    ...
                    const intelResp = await fetch(`${API_BASE}/api/document/${docId}/intelligence`);
                    if (intelResp.ok) {
                ...
                if (failCount >= MAX_FAILS) {
                    ...
                    setParsingStatus(null);
                    setLoading(false);
                    setError('Cannot reach the backend (port 8006). Make sure the SRS-Clarity server is running.');
                } else {
    ...
    const handleFileUpload = async (e) => {
        ...
        if (isPdf) {
            ...
            try {
                ...
                setActiveDocId(data.doc_id);
                // Keep SRS null — let generateDiagram() populate the panel from chunks
                setCustomSrs(null);
                setSelectedSrs('None');
                setParsingStatus(null);
                setLoading(false);
    ...
    const extractNodesFromSrs = useCallback((srsData) => {
        ...
        // 1. Pre-filter: only functional stories with sufficient confidence
        const validStories = stories.filter(s =>
            s.type !== 'contextual_information' &&
            s.type !== 'metadata' &&
            (s.confidence || 0) >= 0.75
        );

        // 2. Actors — only those referenced in valid stories
        const usedActorNames = new Set(validStories.map(s => s.role).filter(Boolean));
        const actorNodes = (srsData.actors || [])
            .filter(a => usedActorNames.has(a))
            .map((a, i) => ({
                id: `actor-${i}`,
                label: a.split(' ')[0],
        ...
        actorNodes.forEach(node => {
            nodeMap[node.fullDescription.replace('Role: ', '')] = node.id;
        });

        // 3. Generate clean, action-oriented labels from raw goal text
        const toActionLabel = (raw) => {
            // Strip common SRS prefixes to get to the core action
            let text = raw
                .replace(/^[\s\-\*\d\.)]+/, '')
                .replace(/^(the )?(system|user|app|application)\s+(must|shall|should|will|can)\s+/i, '')
                .replace(/^(users?\s+)?(must|shall|should|will|can)\s+/i, '')
                .replace(/^(must|shall|should|will|can)\s+/i, '')
                .trim();

            // Take first clause only (before comma, semicolon, period, "to ensure", "in order")
            text = text.split(/[,;.]|(\bto ensure\b)|(\bin order\b)/i)[0].trim();

            // Capitalise first word and take up to 6 words for readability
            const words = text.split(' ').slice(0, 6);
            if (words.length === 0) return raw.split(' ').slice(0, 4).join(' ');
            words[0] = words[0].charAt(0).toUpperCase() + words[0].slice(1);
            return words.join(' ');
        };

        // 4. Build use-case nodes with de-duplication
        const diagType = detectDiagramType(prompt);
        const useCaseNodes = [];
        const seenLabels = new Set();

        validStories.forEach((s, i) => {
            if (useCaseNodes.length >= 20) return;

            const raw = (s.goal || s.raw_text || '').trim();
            if (!raw) return;

            const label = toActionLabel(raw);
            if (!label || label.length < 3) return;

            // De-duplicate on first 18 normalised chars
            const key = label.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 18);
            ...
            useCaseNodes.push({
                id: `uc-${useCaseNodes.length}`,  // sequential — never misaligned
                label,
                fullDescription: raw,
                category
            });
        ...
        // 5. Instructions — built from useCaseNodes directly (no index misalignment)
        const fallbackActor = actorNodes[0];
        const instructions = useCaseNodes.map((uc, i) => {
            const story = validStories[i];
            const actorId = story
                ? actorNodes.find(a => story.role && a.fullDescription.includes(story.role))?.id
                : undefined;
            const from = actorId || fallbackActor?.id;
            return {
                id:    `instr-${i}`,
                text:  `${actorNodes.find(a => a.id === from)?.label || 'Actor'} ➜ ${uc.label}`,
                from,
                to:    uc.id,
                label: 'performs'
            };
    ...
    /* When SRS is selected, populate node panel via backend use-cases endpoint */
    React.useEffect(() => {
        const docId = DOC_ID_MAP[selectedSrs] || (selectedSrs === 'Custom SRS' ? activeDocId : null);

        if (!docId) {
            // No backend doc — clear the panel
            if (selectedSrs === 'None') {
                setAiNodes([]);
                setAiInstructions([]);
            }
        ...
        // Fetch clean use-cases from the smart backend endpoint
        const buildNodes = (data, diagType, runId) => {
            const JOINT_TYPE_MAP = {
                actor:    'uml.Actor',
                use_case: 'uml.UseCase',
                action:   'uml.ActionState',
                process:  'dfd.Process',
                system:   'uml.SystemBoundary',
            };
            ...
            const actors = (data.actors || []).map((a, i) => {
                ...
                return {
                    ...
                    jointType: isSystem ? 'uml.SystemBoundary' : 'uml.Actor',
                };
            });
        ...
        // Fetch clean use-cases from the smart backend endpoint
        setAiNodes([]);  // Show loading state
        fetch(`${API_BASE}/api/document/${encodeURIComponent(docId)}/use-cases`)
            .then(r => r.ok ? r.json() : Promise.reject(r.status))
            .then(data => {
                const diagType = detectDiagramType(prompt);
                const runId = Math.random().toString(36).substring(2, 8);
                const nodes = buildNodes(data, diagType, runId);

                const instructions = [];
                let instrIndex = 0;

                // 1. Build Actor ➜ Use Case instructions
                nodes.filter(n => n.category === 'use_case').forEach((uc) => {
                    const ucData = data.use_cases.find(u => u.id === uc.id || u.label === uc.label);
                    const targetActor = ucData?.actor || '';

                    let actor = nodes.find(a => a.category === 'actor' && targetActor && a.label.toLowerCase() === targetActor.toLowerCase());
                    if (!actor) {
                        actor = nodes.find(a => a.category === 'actor' && targetActor && (targetActor.toLowerCase().includes(a.label.toLowerCase()) || a.label.toLowerCase().includes(targetActor.toLowerCase())));
                    }
                    if (!actor) {
                        actor = nodes.find(a => a.category === 'actor');
                    }

                    if (actor) {
                        instructions.push({
                            id:    `instr-${instrIndex++}-${runId}`,
                            text:  `${actor.label} ➜ ${uc.label}`,
                            from:  actor.id,
                            to:    uc.id,
                            label: 'performs',
                        });
                ...
                // 2. Build <<include>> and <<extend>> relationships from semantic engine
                if (data.relationships && Array.isArray(data.relationships)) {
                    data.relationships.forEach(rel => {
                        ...
                        if (sourceNode && targetNode) {
                            instructions.push({
                                id:    `instr-${instrIndex++}-${runId}`,
                                text:  `${sourceNode.label} ➜ ${targetNode.label} (<<${rel.type}>>)`,
                                from:  sourceNode.id,
                                to:    targetNode.id,
                                label: `<<${rel.type}>>`,
                            });
                ...
                localStorage.setItem('uml_hiloop_nodes', JSON.stringify(nodes));
                localStorage.setItem('uml_hiloop_instr', JSON.stringify(instructions));
            })
            .catch(err => {
                console.warn('[Dashboard] use-cases endpoint failed:', err, '— falling back to local parse');
                // Fallback: minimal local parse if backend is unreachable
                const srs = SRS_SAMPLES[selectedSrs];
                if (srs) {
                    const actors   = (srs.actors || []).map((a, i) => ({ id: `actor-${i}`, label: a.split(' ')[0], fullDescription: `Role: ${a}`, category: 'actor' }));
                    const stories  = (srs.user_stories || []).filter(s => s.type !== 'contextual_information' && (s.confidence||0) >= 0.75);
                    const ucNodes  = stories.slice(0, 15).map((s, i) => ({ id: `uc-${i}`, label: (s.goal||'').split(' ').slice(0,5).join(' '), fullDescription: s.goal||'', category: 'use_case' }));
                    setAiNodes([...actors, ...ucNodes]);
                    setAiInstructions([]);
                }
            });
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [selectedSrs, activeDocId]);
    ...
    const handleDropAll = useCallback(() => {
        ...
        const unplaced = aiNodes.filter(n => !placedNodeIds.has(n.id));
        if (unplaced.length === 0) return;

        const actors      = unplaced.filter(n => n.category === 'actor');
        const systems     = unplaced.filter(n => n.category === 'system');
        const useCases    = unplaced.filter(n => n.category !== 'actor' && n.category !== 'system');

        // ── Layout constants ──────────────────────────────────────────
        const ACTOR_X      = 60;    // actors column X
        const ACTOR_GAP    = 160;   // vertical space between actors
        const UC_START_X   = 300;   // use case grid starts here
        const UC_COL_W     = 210;   // horizontal spacing
        const UC_ROW_H     = 130;   // vertical spacing
        const UC_COLS      = 3;     // use case columns
        const UC_START_Y   = 80;
        const PADDING      = 40;    // System Boundary padding

        // ── Place actors ──────────────────────────────────────────────
        const actorStartY = UC_START_Y + 60;
        actors.forEach((node, idx) => {
            canvas.spawnNode({ ...node, jointType: 'uml.Actor' }, ACTOR_X, actorStartY + idx * ACTOR_GAP);
        });

        // ── Place use cases in a grid ─────────────────────────────────
        useCases.forEach((node, idx) => {
            const col = idx % UC_COLS;
            const row = Math.floor(idx / UC_COLS);
            const x = UC_START_X + col * UC_COL_W;
            const y = UC_START_Y + row * UC_ROW_H;
            let jointType = node.jointType || 'uml.UseCase';
            if (node.category === 'action')  jointType = 'uml.ActionState';
            if (node.category === 'process') jointType = 'dfd.Process';
            canvas.spawnNode({ ...node, jointType }, x, y);
        });

        // ── Auto-place System Boundary wrapping all use cases ─────────
        if (useCases.length > 0) {
            const totalCols = Math.min(useCases.length, UC_COLS);
            const totalRows = Math.ceil(useCases.length / UC_COLS);
            const sbX = UC_START_X - PADDING;
            const sbY = UC_START_Y - PADDING;
            const sbW = totalCols * UC_COL_W + 160 + PADDING;      // +160 for oval width
            const sbH = totalRows * UC_ROW_H + 60  + PADDING * 2;

            // If AI extracted a system node, use its data (id, label) instead of "auto-system-boundary"
            const aiSystem = systems[0]; 
            canvas.spawnNode({
                id:       aiSystem?.id || 'auto-system-boundary',
                label:    aiSystem?.label || 'System',
                category: 'system',
                jointType:'uml.SystemBoundary',
                fullDescription: aiSystem?.desc || 'System boundary',
            }, sbX, sbY, sbW, sbH);
        }

        // ── Auto-draw all connections after a short delay ─────────────
        setTimeout(() => {
            if (canvas.fitContent) canvas.fitContent();
        }, 150);
    ...
    const handleGenerate = async () => {
        ...
        // Use the activeDocId which is set when selecting from the dynamic list or uploading
        const targetDocId = activeDocId || null;
        ...
        try {
            ...
            const graph = await generateDiagram(prompt, targetDocId, activeIntelligence || { raw_text: activeIntelText }, (msg) => {
            ...
            if (graph.entities && graph.entities.length > 0) {
                ...
                const processedInstr = (graph.connections || []).filter(instr => {
                    const fromNode = graph.entities.find(e => e.id === instr.from);
                    const toNode = graph.entities.find(e => e.id === instr.to);
                    return fromNode?.category !== 'system' && toNode?.category !== 'system';
                }).map(instr => ({
                    ...instr,
                ...
                setActiveDiagramType(graph.diagramType || null);
                ...
                localStorage.setItem('uml_hiloop_nodes', JSON.stringify(graph.entities));
                localStorage.setItem('uml_hiloop_instr', JSON.stringify(processedInstr));
                
                setReasoningSteps(prev => [...prev, `✓ Extracted ${graph.entities.length} semantic entities.`]);
            } else {
    ...
    return (
        <div style={{
            ...
            {/* ── Node Panel (Col B + C) ─────────────────────────────────── */}
            {(aiNodes.length > 0 || loading || (selectedSrs && selectedSrs !== 'None')) && (
                <>
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <button 
                                onClick={() => {
                                    ...
                                    const docId = activeDocId || (selectedSrs !== 'None' ? selectedSrs : null);
                                    if (!docId) {
                                        console.warn('Regenerate clicked but no docId found. selectedSrs:', selectedSrs);
                                        return;
                                    }
                                    setAiNodes([]);
                                    setAiInstructions([]);
                                    ...
                                    fetch(`${API_BASE}/api/document/${encodeURIComponent(docId)}/use-cases`)
                                        ...
                                        .then(data => {
                                            ...
                                            const instructions = nodes.filter(n => n.category === 'use_case').map((uc, i) => {
                                                const ucData = data.use_cases[i];
                                                const actor  = nodes.find(a => a.category === 'actor' && ucData?.actor && a.fullDescription.includes(ucData.actor))
                                                               || nodes.find(a => a.category === 'actor');
                                                return actor ? {
                                                    id:    `instr-${i}-${runId}`,
                                                    text:  `${actor.label} ➜ ${uc.label}`,
                                                    from:  actor.id,
                                ...
                                title="Regenerate workspace from SRS"
                                style={{
                            ...
                            >
                                <UploadCloud size={14} style={{ marginRight: '4px' }} /> Regenerate
                            </button>
                        </div>
                        <div style={{ flex: 1, overflow: 'hidden' }}>
                            <NodePanel
                                ...
                                onSpawnNode={(node) => {
                                    if (canvasRef.current) canvasRef.current.spawnNode(node);
                                }}
                                isDark={isDark}
            ...
            <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                ...
                <header style={{
                    ...
                    <TBtn title="Reset canvas (keep sidebar nodes)" onClick={() => {
                        if (window.confirm('Clear canvas? Extracted nodes will stay in the sidebar.')) {
                            ...
                            setPlacedNodeIds(new Set());
                            setConnectedEdgeIds(new Set());
                            localStorage.removeItem('uml_canvas_state');
                        }
                    }}>
                    ...
                    <TBtn title="Wipe everything and start fresh" onClick={() => {
                        if (window.confirm('Wipe everything? This will clear the sidebar and all AI progress.')) {
                            ...
                            setPlacedNodeIds(new Set());
                            setConnectedEdgeIds(new Set());
                            localStorage.removeItem('uml_hiloop_nodes');
                            localStorage.removeItem('uml_hiloop_instr');
                            localStorage.removeItem('uml_canvas_state');
                        }
                    }}>
                ...
                {/* ── Prompt Bar (Moved from footer to top) ──────────────── */}
                <div style={{
                    padding: '12px 18px',
                    background: T.surface,
                    borderBottom: '1px solid ' + T.border,
                        ...
                        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                            ...
                            <div 
                                ...
                                style={{
                                    ...
                                    padding: '0 14px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center',
                            ...
                            <select
                                ...
                                onChange={async (e) => {
                                    ...
                                    // If it's a dynamic stage3 file, we set activeDocId and clear customSrs
                                    const s3 = stage3Files.find(f => f.name === val);
                                    if (s3) {
                                        setActiveDocId(s3.doc_id);
                                        setCustomSrs(null);
                                        
                                        // Fetch raw text for AI context
                                        try {
                                            const txtResp = await fetch(`${API_BASE}/api/stage3/${encodeURIComponent(s3.doc_id)}/content`);
                                            if (txtResp.ok) {
                                                const txt = await txtResp.text();
                                                setRawDocumentText(txt);
                                            }
                                        } catch (e) {
                                            console.warn('Failed to fetch raw SRS content:', e);
                                        }
                                        return;
                                    }
                                    ...
                                    if (val !== 'None' && val !== 'Custom SRS' && !SRS_SAMPLES[val]) {
                                        try {
                                            const resp = await fetch(`${API_BASE}/api/document/${val}/intelligence`);
                                            if (resp.ok) {
                                                const data = await resp.json();
                                                setCustomSrs(data);
                                                setSelectedSrs('Custom SRS');
                                ...
                                style={{
                                    ...
                                    padding: '0 12px', height: '40px', borderRadius: '12px', outline: 'none',
                                    fontSize: '12px', cursor: 'pointer', minWidth: '160px'
                                }}
                            >
                                <option value="None">No SRS Context</option>
                                
                                {stage3Files.map(f => (
                                    <option key={f.doc_id} value={f.name}>
                                        {f.name.replace('_clean.md', '')}
                                    </option>
                                ))}
                            </select>
                            ...
                            <input
                                ...
                                style={{
                                    flex: 1, border: '1.5px solid ' + T.border, background: T.inputBg,
                                    color: T.text, padding: '10px 15px', borderRadius: '12px', outline: 'none',
                            ...
                            <button
                                ...
                                style={{
                                    background: (loading || !prompt.trim()) ? T.border : 'linear-gradient(135deg, #2563eb, #1d4ed8)',
                                    color: 'white', border: 'none', padding: '10px 14px', borderRadius: '12px',
                    ...
                    <DiagramCanvas
                        ...
                        onCellAdded={(id, type) => {
                            if (type === 'link') {
                                ...
                                setPlacedNodeIds(prev => new Set(prev).add(id));
                            }
                        }}
                        onCellRemoved={(id) => {
                                    ...
                                    <div>
                                        ...
                                        <button
                                            onClick={async () => {
                                                ...
                                                try {
                                                    const res = await fetch(`${API_BASE}/api/document/${encodeURIComponent(docId)}/chunks`);
                ...
                {/* ── Footer removed (content moved to top) ── */}
            </main>
        </div>
    );
};
