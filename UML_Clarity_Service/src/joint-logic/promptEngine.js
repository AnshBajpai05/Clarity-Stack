...
const NVIDIA_MODELS = [
    ...
    'meta/llama-3.1-70b-instruct',   // Tier 2 — stable fallback
];
...
- RELATIONSHIPS: "performs" (actor→use_case), "<<include>>" (UC→UC for mandatory sub-steps), "<<extend>>" (UC→UC for optional variants), "generalize"
- SYSTEM BOUNDARY CONNECTIONS: NEVER draw connections (arrows) to or from a "system" node. It is purely an overlay.
- GRANULARITY: You are a senior software architect. Be thorough and exhaustive.
- Extract EVERY distinct user goal as a separate use case node. 
- Do NOT combine multiple goals into one node. If a requirement mentions "logging in and viewing dashboard", that is TWO use cases.
- IDENTIFY ALL ACTORS accurately. Do not invent actors, but don't miss any.
- IDENTIFY ALL PROCESSES (Use Cases). You must find all the "middle things" mentioned.
- IDENTIFY ALL INSTRUCTIONS (Connections). You must draw the complex relationships like <<include>> and <<extend>> between use cases where appropriate!`,
    },
    activity: {
...
export async function retrieveRelevantChunks(docId, userPrompt) {
    if (!docId) {
        console.log('[PromptEngine] No docId — skipping chunk retrieval');
        return [];
    }
    try {
        console.log(`[PromptEngine] Fetching ALL chunks for doc="${docId}"`);
        const res = await fetch(`${BACKEND_URL}/api/document/${encodeURIComponent(docId)}/chunks`);
        if (!res.ok) throw new Error(`Failed to fetch chunks: ${res.status}`);
        
        const data = await res.json();
        let chunks = data.chunks || data || []; 
        
        if (chunks.length === 0) {
            console.log('[PromptEngine] No semantic chunks found — trying raw content fallback');
            const rawRes = await fetch(`${BACKEND_URL}/api/stage3/${encodeURIComponent(docId)}/content`);
            if (rawRes.ok) {
                const text = await rawRes.text();
                chunks = [{
                    heading: "Full Document (Fallback Context)",
                    category: "full_text",
                    body: text,
                    math: [],
                    figures: []
                }];
                console.log('[PromptEngine] ✓ Fallback raw content retrieved');
            }
        }
...
3. NO CROSS-POLLINATION: Use ONLY categories listed for ${diagramType.toUpperCase()}. NEVER use "action" or "decision" in a Use Case diagram. NEVER use "actor" in an Activity diagram.
4. Labels must be SHORT (2-5 words). Descriptions carry the detail.
5. Extract EVERY distinct entity mentioned in the SRS chunks.
6. Create "constraint" entities for ALL mathematical formulas found.
7. Create "note" entities for ALL figure references found.
8. Do NOT invent entities not in the SRS. Quality over quantity.
9. Connections must use UML-standard relationship labels for this diagram type.

## DOCUMENT CONTEXT (Full SRS Text or Summary)
...
function parseSemanticGraph(rawOutput, diagramType) {
    ...
    graph.connections = graph.connections.map((c, i) => ({
        ...c,
        id:   `instr-${i+1}`, // Force uniqueness to prevent UI bugs if LLM repeats IDs
