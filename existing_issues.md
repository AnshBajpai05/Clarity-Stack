...
- **Priority:** P2 · **Effort:** 1–2 wk · **take §5.8** · **Status:** 🟡 PARTIAL (2026-06-30) — the **CI half is started**: `.github/workflows/ci.yml` runs two fast, network-free jobs (backend `pytest` + satellite `node --test`) on every push to `main`/`Clarity_Stack_**` and PRs to `main`. It installs a **minimal `Backend/requirements_ci.txt`** (no torch/transformers — they are lazy in app code and unreached by the tests), **verified green in a clean venv**, which also sidesteps the UTF-16 `requirements_*.txt` problem for the CI path. **Still OPEN:** no Dockerfile/compose (services + Postgres + Mongo + Redis), no lint/typecheck/build stages, no image scanning/Dependabot, and the committed `requirements_*.txt` are still UTF-16. See [`issue_fixed.md §F2`](./issue_fixed.md).
- **Evidence:** launch is `start_project.bat` opening 8 Windows terminal tabs assuming pre-built venvs + local Mongo/Supabase; no Dockerfile/compose, ~~no `.github/`~~ (CI added 2026-06-30), `requirements_*.txt` saved in **UTF-16** (tooling-hostile).
...
- **Priority:** P3 · **Effort:** 2–4 wk · **take §5.6** · **Status:** 🟡 PARTIAL (2026-06-30) — the **harness + golden gate are shipped**. `Backend/eval/` provides content-overlap precision/recall/F1 scoring (reusing the KG token model, §10.3 parity), a **10-case golden set** (raw model output → expected canonical IR), and an OFFLINE runner that scores the **real `prune→parse` extraction pipeline** with no network — wired into pytest as a **CI quality gate** (micro-F1 ≥ 0.95; currently 1.00, 27 bullets, 0 FP/FN, with explicit tests that the scorer *discriminates*). An ONLINE runner profiles the live ensemble for latency (real) + cost (only when priced, never fabricated). This is the CI gate the §11.5 note asked for. **Still OPEN:** labeled `transcript → IR` pairs to score the *live ensemble's extraction accuracy* (the golden set labels raw output, not transcripts), seeds/experiment tracking, golden sets for the legacy 6-model harness, and human-eval. See [`issue_fixed.md §F3`](./issue_fixed.md).
...
* **Current:** A **committed, CI-gated** test suite now exists — Backend `pytest` (77 tests: synthesis/IR validators, `/ask` integration, LLM gateway, logging, plus the §16/§17 engine + AI-eval suites) and Satellite `node --test` (7, content-hash delta) — run on every push/PR by `.github/workflows/ci.yml`. See [`issue_fixed.md §F1/§F2`](./issue_fixed.md).
* **Still Missing:** Frontend unit (`Vitest`), E2E (`Playwright`), API (`Postman/Newman`), and **coverage measurement/thresholds**. Backend integration is in-memory-SQLite only (no live-Postgres CI service yet).
...
* **Current:** An evaluation harness now exists (`Backend/eval/`): a 10-case **golden set** (raw model output → expected IR), **precision/recall/F1** scoring (KG token-model parity), an **offline CI quality gate** on the real extraction pipeline (micro-F1 1.00, with tests proving the scorer discriminates), and an **online latency/cost profiler** for the live ensemble. So "how do you know synthesis is good?" now has a measured, regression-gated answer for the *extraction/validation* layer. See [`issue_fixed.md §F3`](./issue_fixed.md) and §10.10.
* **Still Missing:** **Ground-truth accuracy on labeled `transcript → IR` pairs** (the golden set labels raw output, not transcripts — so end-to-end ensemble *accuracy* isn't scored yet), **human evaluation**, calibration, and experiment tracking across prompt/model versions.
