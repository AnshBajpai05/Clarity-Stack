...
def run_tests():
    print("=" * 60)
    print("  THREATLENS FINAL BOSS 100-URL TEST")
    print("=" * 60)
    
    results = {"benign": {"pass": 0, "fail": 0}, "phishing": {"pass": 0, "fail": 0}, "edge": {"pass": 0, "fail": 0}}
    failures = []

    for category, urls in test_data.items():
        print(f"\n[Category: {category.upper()}]")
        for url in urls:
            try:
                res = requests.post(API_URL, json={"url": url}, timeout=15)
                data = res.json()
                ...
                is_pass = False
                if category == "benign":
                    is_pass = (verdict == "safe")
                elif category == "phishing":
                    is_pass = (verdict == "phishing")
                else: # edge
                    is_pass = True # We count them but don't strictly penalize yet
    ...
    print(f"1. Benign Recall (FPR check): {total_benign - fp}/{total_benign} (FPR: {fpr:.2f}%)")
    ...
    if fpr < 5 and recall >= 90:
        print(">>> ✅ PRODUCTION READY: Passed all criteria.")
    else:
        print(">>> ❌ NEEDS TUNING: Failed core criteria.")

    if failures:
        print("\n[KEY FAILURES]")
        for cat, url, perd, score in failures[:10]:
